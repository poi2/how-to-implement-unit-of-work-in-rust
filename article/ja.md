...
が、それを行うには Rust 固有の所有権やライフタイムの概念を理解していないと実装が難しいこと、また英語、日本語問わず Rust におけるそれらの実装例が少ないため、実装に苦労することが多いです。  
...
- [MUST] 集約を永続化する順番を指定できる
    - 外部キー制約がある場合、正しい順序で CUD を実行する必要があるため
- [SHOULD] 更新後のデータベースの状態をトランザクション内で取得する
    - 更新後の状態をもとに集約を生成・再生成する要件があったときに必要になる
...
例えば `UnitOfWork` trait を実装した `DatabaseClient` が以下のようになっているとします。  
...
- cons
    - Unit of Work を使わない通常の Repository も欲しい場合は Unit of Work と Repository 用の２つの実装を用意する必要がある
        - 間違って Unit of Work を使わずに永続化するミスを防ぐため、通常の Repository の実装を用意しない選択肢も考えられる
